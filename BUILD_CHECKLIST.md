...
- [x] `/dashboard` (earned, synced, wallet balance, unsynced, sync button, clearer prize/global context cards with cleaner leaderboard CTA, no redundant on-chain-earned subcopy, and scoreboard-style match cards with fixture numbers and official recent scorelines) — _API.md / GAME_DESIGN.md_
...
- [ ] `canClaim` flag derivation (rank in tier band + stock + no prior active claim; qualification is implicit since only 10k+ earned wallets appear) — _API.md_
