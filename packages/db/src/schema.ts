...
export const userPoint = pgTable(
  'user_point',
  {
    ...
    tournamentId: integer('tournament_id').notNull(),
    totalPoints: bigint('total_points', { mode: 'bigint' }).default(sql`0`).notNull(),
    lastMatchId: integer('last_match_id'),
    updatedAt: timestamp('updated_at', { withTimezone: true }).defaultNow().notNull(),
  },
  (t) => [
    primaryKey({ columns: [t.wallet, t.tournamentId] }),
    index('user_point_leaderboard_idx').on(t.tournamentId, t.totalPoints.desc()),
    walletCheck('wallet'),
  ],
);
...
export const syncedRecord = pgTable(
  'synced_record',
  {
    ...
    status: text('status').notNull(),
    voucherExpiresAt: timestamp('voucher_expires_at', { withTimezone: true }).notNull(),
    createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
...
export const prize = pgTable(
  ...
  (t) => [uniqueIndex('prize_tournament_tier_key').on(t.tournamentId, t.tierId)],
);
...
export const prizeLeaderboardSnapshot = pgTable(
  'prize_leaderboard_snapshot',
  {
    wallet: text('wallet').notNull(),
    tournamentId: integer('tournament_id').notNull(),
    earnedBalance: numeric('earned_balance', { precision: 78, scale: 0 }).notNull(),
...
export type PrizeLeaderboardSnapshot = typeof prizeLeaderboardSnapshot.$inferSelect;
