...
- [x] Fixed prize card "Rank Locked" bug for eligible users — `TIER` enum in `packages/shared/src/constants.ts` was inverted relative to the rest of the codebase (DB seed, frontend badges, and image filenames all use `1=Grand Prize, 5=Merch`, but the shared enum used `1=Merch, 5=Grand`). `tierForRank()` returned a tier id that never matched any `PrizeCard.tier.tierId`, so every card rendered as "Rank Locked" even when the user was eligible. Flipped the enum to align with the rest of the codebase — no DB migration, no seed change, no image rename. Verification: `pnpm --filter @boundaryline/web typecheck` passes — (2026-04-15)
