...
- [x] Frontend wallet connect + SIWE flow shipped on the landing page — added client-side wallet/auth infra in `apps/web`: `lib/wagmi.ts` (WireFluid-only wagmi config), `components/providers.tsx` (Wagmi + QueryClient + RainbowKit + AuthProvider), `components/auth-provider.tsx` (`useAuth` with injected-wallet connect, automatic switch to WireFluid `92533`, SIWE nonce fetch/sign/verify, JWT persistence in localStorage, wallet mismatch invalidation, logout/unlink), and `lib/api-client.ts` (Bearer-aware fetch helper). Replaced the landing page's fake wallet UI with live client components: `LandingNav`, `HeroWalletActions`, `FinalCtaWalletAction`. Users can now connect, switch chain, sign once, and see linked state on `/`. Verification: `pnpm --filter @boundaryline/shared typecheck`, `pnpm --filter @boundaryline/web typecheck`, `pnpm --filter @boundaryline/web build` all pass — (2026-04-14)
- [x] Backend API surface complete — shipped every remaining v1 route. Admin match/tournament routes, points/sync, claims, leaderboards, prizes, and trophies all landed in `apps/web/app/api/*` with Zod validation, auth checks, and shared error shapes. Verification: `pnpm --filter @boundaryline/db typecheck` and `pnpm --filter @boundaryline/web typecheck` pass — (2026-04-14)
- [x] Players & Teams API shipped — `GET /api/players`, `POST /api/teams`, and `GET /api/teams/me` now enforce team size, salary cap, duplicate checks, active-player validation, and one-team-per-tournament rules. Added `getActiveTournamentId(db)` helper in `packages/db` to prefer `status='active'` and fall back to the newest tournament row. Verification: DB + web typecheck clean — (2026-04-14)
- [x] SIWE nonce storage hardened (cookie → dedicated `siwe_nonce` table) + doc cleanup — replaced the cookie-based nonce with a Postgres-backed `siwe_nonce` table so concurrent tabs each get their own single-use row. Migration `0001_slow_blur.sql` drops `user.siwe_nonce`, creates `siwe_nonce(nonce PK, issued_at, expires_at, consumed_at)` + expires_at index, and `/api/auth/verify` now consumes rows atomically. Docs updated in `docs/DATA_MODEL.md` and `docs/SETUP.md` — (2026-04-14)
- [x] Next.js 16 app scaffolded + SIWE auth shipped — `apps/web` now has the app shell, Tailwind/PostCSS setup, JWT session helpers, SIWE verification, and auth routes (`/api/auth/nonce`, `/api/auth/verify`, `/api/auth/logout`). Verification: `pnpm --filter @boundaryline/web typecheck` clean — (2026-04-14)
- [x] Full-repo consistency sweep for the single-threshold pivot — updated untouched docs and READMEs so every surface now reflects the current model: rank by `balanceOf`, qualify + claim at `earnedBalance ≥ 10k` — (2026-04-14)
