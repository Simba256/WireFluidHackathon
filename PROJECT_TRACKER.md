...
- [x] Unified dashboard match activity into a single vertical feed ordered by match relevance — replaced the split `Recent Performance` / `Upcoming Fixtures` layout in `apps/web/components/dashboard-page.tsx` with one `Match Activity` section that merges the two datasets and sorts them `live` first, then `scheduled`, then `completed` with scored matches trailing below the current/upcoming fixtures. Verification: `pnpm --filter @boundaryline/web typecheck` and `pnpm --filter @boundaryline/web build` both pass — (2026-04-15)
